...
public class WorkspaceController {
        ...
        public ResponseEntity<ApiResponse<List<WorkspaceResponseDto>>> getAllWorkspaces(
                ...
                log.debug("GET /api/workspaces - 워크스페이스 목록 조회, domainId={}, filter={}", domainId, filter);

                String username = authentication != null ? authentication.getName() : "anonymous";
                // If needed, handle anonymous access (e.g. return 401 or empty list)

                List<WorkspaceResponseDto> workspaces = workspaceService.getAllWorkspaces(username, domainId, filter);
        ...
        public ResponseEntity<ApiResponse<WorkspaceResponseDto>> createWorkspace(
                ...
                String username = authentication != null ? authentication.getName() : "anonymous";
                // Enforce login for creation ideally, but for now follow logic
                if (authentication == null) {
                        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ApiResponse.error("로그인이 필요합니다"));
                }

                WorkspaceResponseDto workspace = workspaceService.createWorkspace(requestDto, username);
        ...
        public ResponseEntity<ApiResponse<WorkspaceResponseDto>> updateWorkspace(
                ...
                WorkspaceResponseDto workspace = workspaceService.updateWorkspace(id, requestDto, username);
        ...
        public ResponseEntity<ApiResponse<Void>> deleteWorkspace(@PathVariable Long id,
                ...
                workspaceService.deleteWorkspace(id, username);
